...
- Claude Cowork: `COWORK-PROJECT-INSTRUCTIONS.md`, `COWORK-BRIEF.md`, `tasks/*.md`, `templates/*.md`
...
Cowork 용 결과물에는 프로젝트 `지침` 칸에 복사해서 붙여 넣을 수 있는 `COWORK-PROJECT-INSTRUCTIONS.md` 를 함께 만든다.
이 파일은 Cowork 가 `COWORK-BRIEF.md`, `tasks/`, `templates/` 를 어떻게 참고해야 하는지 알려주는 연결 지침이다.
...
완벽한 하네스를 처음 한 번에 만드는 것은 목표가 아니다.
업무 방식, 산출물 형식, 금지 행동은 실제 작업 중에 바뀔 수 있으므로 처음 결과물은 계속 보강 가능한 구조여야 한다.

## Enhance Existing Harnesses

이미 만든 하네스를 실제로 쓰다 보면 기존 규칙이 맞지 않거나 새 반복 업무가 생길 수 있다.
이때는 `enhance-harness` 스킬을 사용한다.

`enhance-harness` 는 세 가지 상황을 다룬다.

- `Revise`: 기존 규칙을 새 업무 방식에 맞게 바꾼다
- `Extend`: 새 workflow, task, template, example 을 추가한다
- `Patch`: 문구, 승인 경계, 출력 형식 같은 작은 부분만 수정한다

Cowork 의 프로젝트 지침 내용이 바뀌는 경우에는 `COWORK-PROJECT-INSTRUCTIONS.md` 를 갱신하고,
사용자에게 그 내용을 Cowork 프로젝트의 `지침` 칸에 다시 복사하라고 안내한다.

## Install

이 저장소는 Claude Code 와 Codex 용 설치 스킬을 함께 제공한다.

- 이 저장소 안의 Claude Code 용 원본:
  - `.claude/skills/harness-builder/SKILL.md`
  - `.claude/skills/enhance-harness/SKILL.md`
- 이 저장소 안의 Codex 용 원본:
  - `.agents/skills/harness-builder/SKILL.md`
  - `.agents/skills/enhance-harness/SKILL.md`

이 저장소 안의 `.claude/` 와 `.agents/` 는 source repo 의 배포용 원본이다.
사용자가 실제로 설치해야 하는 위치는 각자의 홈 폴더 아래 skill 폴더다.
...
```text
/Users/<사용자이름>/.claude/skills/harness-builder/SKILL.md
/Users/<사용자이름>/.claude/skills/enhance-harness/SKILL.md
```
...
```text
C:\Users\<사용자이름>\.claude\skills\harness-builder\SKILL.md
C:\Users\<사용자이름>\.claude\skills\enhance-harness\SKILL.md
```

Windows 에서 WSL 로 Claude Code 를 쓰는 경우에는 WSL 안의 홈 폴더를 기준으로 한다.

```text
/home/<사용자이름>/.claude/skills/harness-builder/SKILL.md
/home/<사용자이름>/.claude/skills/enhance-harness/SKILL.md
```
...
```text
/Users/<사용자이름>/.codex/skills/harness-builder/SKILL.md
/Users/<사용자이름>/.codex/skills/enhance-harness/SKILL.md
```

Windows 에서 Codex 를 WSL 로 쓰는 경우:

```text
/home/<사용자이름>/.codex/skills/harness-builder/SKILL.md
/home/<사용자이름>/.codex/skills/enhance-harness/SKILL.md
```

Windows 네이티브 환경에서 Codex 가 Windows 홈 폴더를 사용하는 경우:
...
```text
C:\Users\<사용자이름>\.codex\skills\harness-builder\SKILL.md
C:\Users\<사용자이름>\.codex\skills\enhance-harness\SKILL.md
```

### 설치 후 확인

설치 후 폴더 구조는 아래처럼 보여야 한다.
...
그다음 하네스를 만들고 싶은 프로젝트 폴더를 Claude Code 또는 Codex 로 연 뒤 `/harness-builder` 를 실행한다.

## How To Use

1. 위 설치 위치에 `harness-builder` 폴더와 `enhance-harness` 폴더를 복사한다.
...
- 기존 하네스의 규칙을 바꾸거나 새 workflow 를 추가할 때는 `/enhance-harness` 를 실행
...
- `.claude/skills/enhance-harness/SKILL.md`: Claude Code 용 하네스 보강 스킬
- `.agents/skills/harness-builder/SKILL.md`: Codex 설치용 스킬
- `.agents/skills/enhance-harness/SKILL.md`: Codex 용 하네스 보강 스킬
- `.gitignore`: 개발용 무시 목록
